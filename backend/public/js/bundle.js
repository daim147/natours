...
var displayMap = function displayMap(locations) {
  ...
  locations.forEach(function (loc) {
    new mapboxgl.Marker({
      anchor: 'bottom'
    }).setLngLat(loc.coordinates).setPopup(new mapboxgl.Popup().setHTML("<p>Day ".concat(loc.day, ": ").concat(loc.description, "</p>"))).addTo(map).togglePopup();
  });
  map.fitBounds(locations.map(function (loc) {
    ...
    return _regeneratorRuntime().wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            _context.prev = 0;
            url = type === 'password' ? '/api/v1/auth/updateMyPassword' : '/api/v1/user/updateMe';
            _context.next = 4;
            return (0, _axios.default)({
              method: 'PATCH',
              url: url,
